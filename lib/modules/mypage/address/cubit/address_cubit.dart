...
class AddressCubit extends Cubit<AddressState> {
  ...
  Future<void> createAddress(Address address) async {
    ...
    ApiResult<Address> apiResult = await _addressRepository.createAddress(newAddress);

    apiResult.when(success: (Address? response) {
      if(response!.isDefault!) {
        emit(state.copyWith(
          addresses: state.addresses!.map((adr) => adr.copyWith(isDefault: false)).toList() +[response]
        ));
      } else {
        emit(state.copyWith(
  ...
  Future<void> deleteAddress(int addressId) async {
    ApiResult<dynamic> apiResult = await _addressRepository.deleteAddress(addressId);
  ...
  Future<void> updateDefaultAddress(int addressId) async {
    ApiResult<dynamic> apiResult = await _addressRepository.updateDefaultAddress(addressId);
